...
Stage 3 must not wait for human review or human approval. Human input is needed only for missing credentials, platform permissions that prevent every available automated approval/admin/force-merge path, repeated unresolved blockers, failing validation that cannot be fixed, merge conflicts, or a required change to accepted design decisions.
...
3. For each planned phase, in order:
   ...
   - Use `pr_manager` to draft the GitHub PR body from the implementation plan, open the PR with explicit `--base main`, `--head agent/<roadmap-slug>-p<n>-<phase-slug>`, and title flags, verify the opened PR target with `gh pr view`, monitor checks, submit an automated approval when review-only protection requires approval, automatically squash-merge when gates pass, and clean up after merge.
...
- GitHub allows squash merge, or review-only protection can be satisfied by automated approval, admin merge, or force merge after automated gates pass.
...
The PR manager should use auto-merge when checks are pending and direct squash merge when checks have already passed. If branch protection blocks solely on review approval requirements, the PR manager must submit an automated approving review after automated review and validation gates pass, then retry merge. If GitHub rejects that approval because of self-approval or permission rules, the PR manager must use admin merge or force merge when available. Stop only when GitHub permissions prevent every automated approval/admin/force-merge path. Do not merge known failing validation, wrong-target PRs, unresolved conflicts, unresolved implementation/review blockers, or changes outside the accepted plan.
