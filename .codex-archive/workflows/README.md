...
- `stage-2-master-plan.md`: draft, get explicit intent/functionality approval, discuss only impactful unsettled design decisions, refine, review, and quality-gate one accepted roadmap work package into a decision-complete master implementation plan.
...
Human acceptance is required after Stage 1 behavior/design planning and after Stage 2 master-plan drafting/refinement. Stage 3 proceeds autonomously one phase at a time after the master plan passes its quality gate. Human review is not a merge gate. Stage 3 submits automated approval or uses admin/force merge after automated gates pass when branch protection is blocking only on review approval. Stage 3 stops only when authentication, platform permissions preventing every automated approval/admin/force-merge path, repeated blockers, unresolved checks, merge conflicts, or a required change to accepted design decisions prevents autonomous progress.
...
3. Run Stage 2 directly: read context, give a concise readout, draft `docs/implementation/<roadmap-slug>/master-plan.md`, and get explicit maintainer approval for overall intent/functionality before design discussion.
4. Classify design decisions. Record implementation details and strong recommendations in the plan; discuss only impactful decisions without strong recommendations, plus any recommendation the maintainer asks to discuss.
5. Refine the master plan after design discussion, then run the master-plan quality gate with `.codex/prompts/master-plan-review.md`, `.codex/prompts/master-plan-quality-refine.md` if needed, and a confirmation review; record results in `master-plan.md`.
6. Resolve any final focused questions surfaced while drafting/refining `implementation-plan.md`.
7. After the quality gate passes, run Stage 3.
8. Stage 3 creates or updates `docs/implementation/<roadmap-slug>/implementation-plan.md`.
9. Implement phases sequentially. For each phase, choose the standard or fast pathway, run the planning/coding/check loop, open the PR, submit any required automated approval, auto/admin/force merge after automated gates pass, delete the branch, clean up the worktree, then start the next phase.
