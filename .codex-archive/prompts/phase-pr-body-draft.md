...
10. If branch protection blocks solely on review approval requirements, submit an automated approving PR review after automated validation and pathway gates pass, then retry merge. If GitHub rejects that approval because of self-approval or permission rules, use admin merge or force merge when available.
...
- If auth, checks, conflicts, or permissions prevent every automated approval/admin/force-merge path, record exact evidence in `implementation-plan.md` and stop.
