...
| `MethodOutput` | Patch-like method result that maps explicit `FieldLocator`s to prediction or representation fields. | Not a `Batch`, export result, metric update, loss value, trainer step output, or hidden mutation request. |
...
| `StepOutput` | Learner result carrying opaque predictions, optional backwardable objective, loss/objective terms, metric values, diagnostics, metadata, and provenance. | Not a trainer result, export manifest, materialized prediction table, or post-prediction pipeline executor. |
| `TrainingPlan` | Caller-assembled training inputs such as batch iterables, loop limits, backend hooks, and observe-only event/profiler hooks. | Not a learner container, datasource scanner, dataloader builder, project config, artifact directory, or generic `engine_config`. |
