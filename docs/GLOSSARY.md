...
| `TrainingProfile` | Engine-neutral aggregate of event logs, scalar spans, resource traces, probe results, writer/monitor lifecycle records, checkpoint evidence, and primitive decisions. | Not a raw profiler trace, framework trainer state, tensor capture, scheduler, or artifact writer. |
| `ResourceTrace` | Timestamped primitive resource samples for a metric kind, unit, cadence, rank/device/process attribution, and unavailable or ambiguous probe evidence. | Not a single whole-run performance score, mandatory GPU/system dependency, hidden synchronization rule, or backend profiler object. |
| `CheckpointRef` | Primitive checkpoint identity and status evidence that can point to a path or URI without owning payload serialization. | Not checkpoint contents, optimizer state, an artifact store record, or a workflow resume command. |
| `CheckpointCatalog` | In-memory evidence index used to select checkpoint refs by latest, best, rewind, timestamp, failure, final, or explicit criteria. | Not a database, dashboard, storage backend, retention daemon, or artifact lifecycle manager. |
| `ExperimentTierSpec` | Descriptive debug/smoke/signal/comparison/full validation breadth using the same execution path at different scale with expected evidence and fingerprints. | Not a scheduler policy, CI speed threshold, cost dashboard, alternate fake path, or downstream workflow runtime. |
| `RestartState` | Snapshot linking a checkpoint ref to loader/materialization/data-path/profile fingerprints, completion markers, and compatibility notes. | Not a checkpoint payload, restore implementation, job retry policy, artifact lifecycle record, or generic resume engine. |
