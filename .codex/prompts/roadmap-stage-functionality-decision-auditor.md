...
2. Review the resolved functionality-agreement queue, behavior confirmation,
   ...
   findings, assumptions, and deferrals as one coherent plan.
...
4. Reopen only the relevant agreement-queue item as `needs maintainer
   ...
   scientific/workflow, downstream-project, or refactor risk that requires
   maintainer judgment. If the issue has a clear default, record it as a
   concern or recommendation instead of reopening it for discussion.
...
10. Update only `docs/roadmap/stage-<N>/planning.md`, especially the agreement
    queues, `Design Decision Triage`, `Functionality And Decision Audit`, and
